...
const {uploadSingle} = require("../middleware/uploadmiddlerware/upload");
...
router.patch("/profile",uploadSingle('profile_pic'),async(req,res)=>{
  try {
    ...
    const user= await User.update({firstName,lastName,email,phone,gender,profile_pic:req.file.filename},{where:{user_Id:req.body.user_Id}})
    if(user[0]===1){
    ...
    return res.status(200).send({message:"User not found" , isUpdated:user[0]})
...
try {
  const user= await User.update({...req.body},{where:{user_Id:req.body.user_Id}});
  if(user[0]===1){
    const updatedUser= await User.findByPk(req.body.user_Id);
    return res.status(200).send(updatedUser)
  }
   ...
   try {
    ...
    //  console.log('user exist ',dataValues)
     if(dataValues.user_Id){
      ...
      const passwordResetToken=  jwt.sign({user_Id: dataValues.user_Id}, process.env.RESET_PASSWORD_SECRET_KEY, {expiresIn:'10m'});
      ...
      const updateUserResetPasswordToken = await User.update({resetPasswordToken:passwordResetToken},{where:{user_Id:dataValues.user_Id, }});
      ...
      if(updateUserResetPasswordToken[0]==1){
         ...
         return res.status(200).send({message:'Password reset mail successfully sent'})
      }
       return res.status(400).send({message:"Something went wrong "})
     }
     return res.status(400).send({message:"No user exists with the email address?"})
   } catch (error) {
...
router.post("/register/verify/updatePassword",async(req,res)=>{
   try {  
    //?? exctracting token and new password from the request body
     ...
     if(token){
       jwt.verify( token , process.env.RESET_PASSWORD_SECRET_KEY , async (err, user)=>{
          if(err){
            return res.status(400).send({message:'Token is expired or not valid'})
          }
          //? verifying the token with updated resetPasswordToken field in the user table 
          const  {dataValues}=  await User.findOne({where:{resetPasswordToken:token}});
           if(dataValues){
            //** now update the new password with the user's old password */
            const  isPasswordUpdated= await User.update({password:hashPassword(password)},{where:{user_Id:dataValues.user_Id}});
            //! checking if updation is successful
             if(isPasswordUpdated[0]===1){
               return res.status(200).send({message:'Password updated successfully'});

             }
             return res.status(400).send({message:'Error while updating the password'})
           }
           return res.status(400).send({message:'Token is not matched with the resetPasswordToken?'})
       })
     }
    ...
    try {
       ...
       if(token){
         jwt.verify(token , process.env.JWT_SECRET_KEY, async(err, user)=>{
           ...
           const  isUserAccountActive = await User.update({is_Active:1}, {where:{user_Id}});
