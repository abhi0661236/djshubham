...
const verificationMail = async ({
  ...
  const welcomeMail = async ({ from, to,alternatives,html }) => {
    await sandMail({
      from,
      to,
      subject: ` Registeration verification  email  `,
      html,
     alternatives
  ...
  const sendPasswordResetVerificationEmail = async({from, to, email, token})=>{
    await sandMail({
       ...
       html:`
        ...
        <a href='http://192.46.209.205:3003/user/forgotPassword/update/${token}'>Reset password here </a>
       `
    })
}
